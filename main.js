const dictionary = {
    ...
    '전': {
        start: ['전능하신 하나님의 손길이 당신의 모든 연약함을 강건하게 만져주시고,', '전심으로 주를 찾는 당신에게 하늘의 보화가 열리는 기적이 임하며,', '전례 없는 주님의 특별한 은총이 당신의 일상을 매일 새롭게 변화시켜서,'],
        mid: ['전부이신 예수 그리스도를 삶의 주인으로 모시고 당당히 살아가고,', '전신 갑주를 입고 믿음의 선한 싸움에서 넉넉히 승리하는 용사가 되어,', '전 세계에 주의 사랑을 흘려보내는 거룩하고 아름다운 축복의 통로가 되어,'],
        end: ['전능하신 주님의 팔이 당신을 영원토록 든든하게 지켜주실 것입니다.', '전심을 다해 주님을 사랑하는 당신의 앞길에 하늘 복이 가득하길 빌어요.', '전례 없이 빛나는 주의 자녀로 세상에서 승리하는 당신이 되소서.']
    },
    '진': {
        start: ['진리의 말씀이 당신의 앞길에 등불이 되어 실족하지 않게 늘 지켜주시고,', '진실한 고백을 기뻐하시는 주님 앞에 날마다 순전한 아이처럼 서는 당신 되길,', '진귀한 보물보다 당신을 더 귀히 여기시는 하나님의 마음을 깊이 느끼며,'],
    ...
    '희': {
        ...
        mid: ['희생과 사랑으로 헌신하는 당신의 손길을 통해 주의 나라가 확장되고,', '희망의 항구로 인도하시는 성령님과 손잡고 인생의 바다를 평안히 항해하여,', '희년의 기쁨과 자유함이 당신의 영혼 깊은 곳에서부터 샘솟아 올라서,'],
        end: ['희망 가득한 당신의 앞날에 주님의 특별한 은총이 늘 함께하시길 빌어요.', '희망의 주님만을 바라보며 끝까지 믿음의 경주를 완주하는 승리자가 되소서.', '희귀하고 존귀한 당신의 존재가 주님 안에서 더욱 찬란히 빛나길 축복합니다.']
    },
    '제': {
    ...
    '유': {
        start: ['유난히 따뜻한 주의 은혜가 당신의 모든 발걸음을 감싸 안고,', '유일하신 하나님만을 온전히 경외하는 믿음의 뿌리가 깊어져서,', '유연하고 부드러운 마음으로 주님의 사랑을 세상에 널리 실천하며,'],
        mid: ['유리바다처럼 맑은 주의 평강이 당신의 모든 생각을 덮어주고,', '유익하고 복된 삶으로 많은 이들에게 주님의 사랑을 전하는 통로가 되어,', '유달리 행복한 웃음소리가 당신의 가정과 일터에서 끊이지 않게 되어,'],
        end: ['유리처럼 투명한 주의 은혜가 당신의 심령에 영원히 머물길 축복합니다.', '유달리 빛나는 주의 자녀로 세상 속에서 날마다 승리하시길 기도해요.', '유구한 시간 속에서도 변치 않는 주님의 사랑을 평생 누리며 사소서.']
    },
    '은': {
    ...
    '하': {
        start: ['하늘 문을 활짝 여시고 쏟아지는 하늘의 복을 당신이 가득 받으시길,', '하나님의 형상을 닮은 당신은 이 세상에서 가장 소중하고 귀한 주님의 편지이며,', '하늘 가득한 주님의 영광이 당신의 앞길을 찬란하게 비추어주는 아침,'],
        mid: ['하루의 시작과 끝이 오직 감사의 고백으로 가득히 채워지는 기쁨을 경험하고,', '하늘 소망 가슴에 품고 주님과 함께 예쁜 꽃길을 사뿐히 걸어가는 주인공 되어,', '하나님의 큰 기쁨이 되는 당신의 모든 날을 주님이 응원하시고 축복하셔서,'],
        end: ['하늘 가득한 주님의 영광이 당신의 앞길을 평생토록 찬란하게 비추길 기도합니다.', '하나님 보시기에 가장 아름다운 당신의 모든 날들을 주님의 이름으로 축복해요.', '하늘 소망 붙들고 주와 함께 영원히 행복한 주인공으로 살아가소서.']
    },
    '윤': {
...
function generateSinglePoem(name, globalUsed) {
    ...
    characters.forEach((char, index) => {
        ...
        if (dictionary[char]) {
            ...
            let available = options.map((_, i) => i).filter(i => !usedIndices[key].has(i));
            if (available.length === 0) { usedIndices[key].clear(); available = options.map((_, i) => i); }
            const randIdx = available[Math.floor(Math.random() * available.length)];
            usedIndices[key].add(randIdx);
