const dictionary = {
    ...
    '장': {
        start: ['장막 터를 넓히시는 하나님의 축복이 당신의 삶에 가득하고,', '장성한 분량까지 당신의 믿음이 쑥쑥 자라나길 소망하며,'],
        mid: ['장차 나타날 영광을 기대하며 오늘의 어려움을 이겨내고,', '장인처럼 정교하게 당신의 삶을 빚으시는 주의 손길을 믿어,'],
        end: ['장대하게 뻗어가는 믿음의 가문을 이루시길 축복합니다.', '장소에 상관없이 어디서나 주를 예배하는 자 되소서.']
    },
    '임': {
        start: ['임마누엘 하나님이 당신의 모든 순간에 함께 계심을 믿고,', '임재의 깊은 바다로 나아가 주님의 세밀한 음성을 듣기를,'],
        mid: ['임명하신 사명의 자리에서 묵묵히 충성하며 꽃을 피우고,', '임시적인 세상보다 영원한 하늘 보화를 더 귀하게 여겨서,'],
        end: ['임금 되신 주님의 통치 아래서 참된 평강을 누리소서.', '임하신 성령의 불길이 당신의 영혼을 따뜻하게 달구길 빌어요.']
    },
    '한': {
...
function getJosa(char, type) {
    ...
    if (code < 44032 || code > 55203) return ''; // 한글 아님
...
function generatePoem(name) {
    ...
    const usedIndices = {}; // 중복 방지용 (글자별로 몇 번째 문구를 썼는지 추적)
    ...
    characters.forEach((char, index) => {
        ...
        if (dictionary[char]) {
            ...
            do {
                ...
                attempts++;
            } while (usedIndices[char].has(randIdx) && attempts < 10);
